<?php
    ...
    $sql = "SELECT Word_Count FROM readingskills_material WHERE Language_Material = 'Filipino' ORDER BY Material_ID DESC LIMIT 1";
    ...
    $stmt = $conn->prepare("UPDATE readingskills_result SET Filipino_Video = ?, Filipino_ReadingTime = ?, Filipino_ReadingTimeSpeed = ?, Filipino_ReadingStatus = ? WHERE StudentID_Number = ?");
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows === 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO readingskills_result (StudentID_Number, Filipino_Video, Filipino_ReadingTime, Filipino_ReadingTimeSpeed) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("isii", $studentID, $videoLocation, $readingTime, $readingTimeSpeed);
            if ($stmt->execute()) {
