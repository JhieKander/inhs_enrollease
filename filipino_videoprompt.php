<?php
    include 'header.php';
    include 'PHP/filipinomaterial.php';
...
<body>
                    ...
                    <div class="form-row">
                        ...
                        <div class="tainer">
                            ...
                            <?php
                                ...
                                $disabled = empty($row['Filipino_Video']) ? 'disabled' : '';
                            ?>
                            <button class="proceed-button" id="proceed-button" <?php echo $disabled; ?> onclick="window.location.href='filipino_passage.php'">Proceed</button>
                            <script>
                                // Re-check video status after recording
                                document.getElementById('stopRecording').addEventListener('click', function() {
                                    // Use AJAX to check the video status without reloading the page
                                    fetch('PHP/video_check_status.php')
                                        .then(response => response.json())
                                        .then(data => {
                                            const proceedButton = document.getElementById('proceed-button');
                                            proceedButton.disabled = !data.videoExists; // Enable or disable button based on video existence
                                        })
                                        .catch(error => console.error('Error fetching video status:', error)); // Handle errors
                                });
                            </script>
                        </div>
                    </div>
    ...
    <script src="JavaScript/tele-video.js"></script>
